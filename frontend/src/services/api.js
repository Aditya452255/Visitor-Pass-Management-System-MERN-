...
const getAuthHeaders = (isJson = true) => {
  ...
  // Dev-only logging to help debug authorization issues (no token printed)
  if (process.env.NODE_ENV !== 'production') {
    try {
      // only log whether a token exists and the stored role
      const raw = localStorage.getItem('user');
      const stored = raw ? JSON.parse(raw) : null;
      // eslint-disable-next-line no-console
      console.debug('[api] getAuthHeaders -> role:', stored?.role, 'hasToken:', !!(stored && stored.token));
    } catch (e) {
      // ignore
    }
  }
...
export const visitorAPI = {
  ...
  // use public register endpoint so visitors can pre-register without auth
...
export const appointmentAPI = {
  ...
  create: async (data) => {
    ...
    const headers = user && user.token ? getAuthHeaders() : { 'Content-Type': 'application/json', Accept: 'application/json' };
  ...
  approve: async (id) => {
    if (process.env.NODE_ENV !== 'production') {
      // eslint-disable-next-line no-console
      console.debug('[api] approve called for id:', id);
    }
    ...
    if (process.env.NODE_ENV !== 'production') {
      // eslint-disable-next-line no-console
      console.debug('[api] approve response status:', response.status, 'data:', data);
    }
    if (!response.ok) {
      const err = (data && data.error) ? data.error : (typeof data === 'string' ? data : JSON.stringify(data));
      throw new Error(err);
...
export const passesAPI = {
  ...
  verify: async (passNumber) => {
    const response = await fetch(`${API_URL}/passes/verify/${passNumber}`, {
      headers: getAuthHeaders()
    });
    return response.json();
  },
