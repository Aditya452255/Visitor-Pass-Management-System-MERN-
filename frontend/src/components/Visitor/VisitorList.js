...
const PLACEHOLDER_IMG = '/images/placeholder.png'; // ensure this exists in public/
          ...
          <tbody>
            {(!visitors || visitors.length === 0) ? (
              ...
              visitors.map((visitor) => {
                ...
                // Defensive handling: visitor.photo might be missing or not a string
                let photoPath = PLACEHOLDER_IMG;
                if (visitor && typeof visitor.photo === 'string' && visitor.photo.trim() !== '') {
                  photoPath = visitor.photo.startsWith('http')
                    ? visitor.photo
                    : `http://localhost:5000/${visitor.photo.replace(/^\/+/, '')}`;
                }
