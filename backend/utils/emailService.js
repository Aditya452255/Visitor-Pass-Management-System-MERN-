...
const sendAppointmentConfirmation = async (appointment, visitor, host, qrCodeDataURL) => {
    console.log('[EmailService] Preparing to send appointment confirmation.');
    console.log(`[EmailService] Visitor Email: ${visitor.email}`);
    console.log(`[EmailService] Host Name: ${host.name}`);
    console.log(`[EmailService] QR Code available: ${!!qrCodeDataURL}`);
  ...
  `;
    ...
    try {
        ...
        console.log('[EmailService] Mail options preview (to, subject, attachments count):', {
            to: mailOptions.to,
            subject: mailOptions.subject,
            attachments: mailOptions.attachments.map(a => ({ filename: a.filename, hasContent: !!a.content, path: a.path || null, cid: a.cid || null }))
        });

        const info = await transporter.sendMail(mailOptions);
        console.log(`[EmailService] Appointment confirmation with QR code sent to ${visitor.email}. Message ID: ${info.messageId}`);
        return true;
    } catch (error) {
