...
const checkIn = async (req, res) => {
  try {
    ...
    if (!passId || !visitorId) {
      return res.status(400).json({ error: 'passId and visitorId are required' });
    }
    ...
    const checkLog = await CheckLog.create({
      ...
      checkInBy: req.user && req.user._id ? req.user._id : null,
      temperature,
      deviceInfo,
      notes,
      location
...
const checkOut = async (req, res) => {
  try {
    ...
    checkLog.checkOutBy = req.user && req.user._id ? req.user._id : null;
